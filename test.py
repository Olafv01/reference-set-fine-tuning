...
def test_efficient_ram_usage(args, eval_ds, model, test_method="hard_resize"):
    ...
    if "is_trainref" in dir(args):
        if args.is_trainref:
            ...
            np.save("predections_{}_on_{}".format(args.resume.split(".")[0].split('/')[-1],args.dataset_name.split("/")[0]),predictions)
...
def val(args, eval_ds, model, test_method="hard_resize", pca=None):
    ...
    model = model.eval()
    with torch.no_grad():
        logging.debug("Extracting database features for evaluation")
        # For database use "hard_resize", although it usually has no effect because database images have same resolution
        eval_ds.test_method = "hard_resize"
        database_subset_ds = Subset(eval_ds, list(range(eval_ds.database_num)))
        database_dataloader = DataLoader(dataset=database_subset_ds, num_workers=args.num_workers,
        ...
        for inputs, indices in database_dataloader:
            features = model(inputs.to(args.device))
        ...
        logging.debug("Extracting queries features for evaluation")
        queries_infer_batch_size = 1 if test_method == "single_query" else args.infer_batch_size
        eval_ds.test_method = test_method
        queries_subset_ds = Subset(eval_ds, list(range(eval_ds.database_num, eval_ds.database_num+eval_ds.queries_num)))
        queries_dataloader = DataLoader(dataset=queries_subset_ds, num_workers=args.num_workers,
                                        batch_size=queries_infer_batch_size, pin_memory=(args.device == "cuda"))
        for inputs, indices in queries_dataloader:
            if test_method == "five_crops" or test_method == "nearest_crop" or test_method == 'maj_voting':
                inputs = torch.cat(tuple(inputs))  # shape = 5*bs x 3 x 480 x 480
...
def test(args, eval_ds, model, test_method="hard_resize", pca=None):
    ...
    model = model.eval()
    with torch.no_grad():
        logging.debug("Extracting database features for testing")
        # For database use "hard_resize", although it usually has no effect because database images have same resolution
        eval_ds.test_method = "hard_resize"
        database_subset_ds = Subset(eval_ds, list(range(eval_ds.database_num)))
        database_dataloader = DataLoader(dataset=database_subset_ds, num_workers=args.num_workers,
        ...
        logging.debug("Extracting queries features for testing")
        queries_infer_batch_size = 1 if test_method == "single_query" else args.infer_batch_size
        eval_ds.test_method = test_method
        queries_subset_ds = Subset(eval_ds, list(range(eval_ds.database_num, eval_ds.database_num+eval_ds.queries_num)))
        queries_dataloader = DataLoader(dataset=queries_subset_ds, num_workers=args.num_workers,
...
#             if args.augments and not args.dataset_name=="amstertime":
#                 inputs=combining_methods(inputs)
